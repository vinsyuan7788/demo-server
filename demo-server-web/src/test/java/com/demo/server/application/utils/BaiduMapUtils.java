...
public class BaiduMapUtils {
     ...
     *  批量算路（步行）
     */
    private static final String BAIDU_ROUTE_MATRIX_WALKING_URL = "https://api.map.baidu.com/routematrix/v2/walking";
    private static final String BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE = "distance";
    private static final String BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE_VALUE = "value";
    ...
    public static List<GeoCoordinateInfo> getDistanceInbetweeen(String ak, List<GeoCoordinateInfo> origins, List<GeoCoordinateInfo> destinations, RestTemplate restTemplate) {
        String urlOfRouteMatrixOfWalking = getUrlOfRouteMatrixOfWalking(ak, origins, destinations, SupportedCoordinateTypeEnum.BD09LL, OutputFormatEnum.JSON);
        String response = restTemplate.getForObject(urlOfRouteMatrixOfWalking, String.class);
        return parseResponseOfRouteMatrixOfWalking(response, OutputFormatEnum.JSON);
    }
     ...
     * @return
     */
    private static String getUrlOfRouteMatrixOfWalking(String ak, List<GeoCoordinateInfo> origins, List<GeoCoordinateInfo> destinations, SupportedCoordinateTypeEnum coordinateTypeEnum, OutputFormatEnum outputFormatEnum) {
        if (StringUtils.isBlank(ak) || CollectionUtils.isEmpty(origins) || CollectionUtils.isEmpty(destinations)) {
            throw new BusinessException("发现有入参为空，无法拼接请求链接");
        }
        String url = BAIDU_ROUTE_MATRIX_WALKING_URL
                // 开发者账号的申请 key
                + "?ak=" + ak
                // 默认 JSON 格式
                + "&output=" + parseOutput(outputFormatEnum)
        ...
        log.info("=== getUrlOfRouteMatrixOfWalking | 获取请求链接 | 内容: {} ===", url);
        return url;
    }
                            ...
                            if (result.containsKey(BAIDU_GEO_CODING_URL_RESULT_LOCATION)) {
                                ...
                                if (location.containsKey(BAIDU_GEO_CODING_URL_RESULT_LOCATION_LONGITUDE)) {
                                    ...
                                    if (StringUtils.isNotBlank(longitude)) {
                                        ...
                                        log.error("=== parseResponseOfGeoCoding | 字段{}的值为空 ===", BAIDU_GEO_CODING_URL_RESULT_LOCATION_LONGITUDE);
                                    }
                                } else {
                                ...
                                if (location.containsKey(BAIDU_GEO_CODING_URL_RESULT_LOCATION_LATITUDE)) {
                                    ...
                                    if (StringUtils.isNotBlank(latitude)) {
                                        ...
                                        log.error("=== parseResponseOfGeoCoding | 字段{}的值为空 ===", BAIDU_GEO_CODING_URL_RESULT_LOCATION_LATITUDE);
                                    }
                                } else {
     ...
     * @return
     */
    private static List<GeoCoordinateInfo> parseResponseOfRouteMatrixOfWalking(String response, OutputFormatEnum outputFormatEnum) {
        if (outputFormatEnum == null) {
            throw new BusinessException("响应格式为空，无法解析");
        }
        log.info("=== parseResponseOfRouteMatrixOfWalking | 获取响应信息 | 内容: {} ===", response);
        switch (outputFormatEnum) {
            case JSON:
                        ...
                        if (jsonObject.containsKey(BAIDU_URL_RESPONSE_RESULT)) {
                            ...
                            if (size > 0) {
                                for (int i = 0; i < size; i++) {
                                    ...
                                    if (distanceAndDurationInfo.containsKey(BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE)) {
                                        JSONObject distanceInfo = distanceAndDurationInfo.getJSONObject(BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE);
                                        if (distanceInfo.containsKey(BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE_VALUE)) {
                                            String distanceMeters = distanceInfo.getString(BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE_VALUE);
                                            if (StringUtils.isNotBlank(distanceMeters)) {
                                                geoCoordinateInfo.setDistance(new BigDecimal(distanceMeters));
                                            } else {
                                                log.error("=== parseResponseOfRouteMatrixOfWalking | 字段{}的值为空 ===", BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE_VALUE);
                                            }
                                        } else {
                                            log.error("=== parseResponseOfRouteMatrixOfWalking | 不存在字段{} ===", BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE_VALUE);
                                        }
                                    } else {
                                        log.error("=== parseResponseOfRouteMatrixOfWalking | 不存在字段{} ===", BAIDU_ROUTE_MATRIX_WALKING_URL_RESULT_DISTANCE);
                                    }
                                    geoCoordinateInfoList.add(geoCoordinateInfo);
                                }
                            } else {
                                log.error("=== parseResponseOfRouteMatrixOfWalking | 结果集为空 ===");
                            }
                        } else {
                            log.error("=== parseResponseOfRouteMatrixOfWalking | 不存在字段{} ===", BAIDU_URL_RESPONSE_RESULT);
                        }
                    } else {
                        log.error("=== parseResponseOfRouteMatrixOfWalking | 响应状态不正常 | 状态: {} ===", status);
                        throw new BusinessException("响应状态不正常（" + status + "）");
                    }
                } else {
                    log.error("=== parseResponseOfRouteMatrixOfWalking | 不存在字段{} ===", BAIDU_URL_RESPONSE_STATUS);
                }
                    ...
                    if (BAIDU_URL_RESPONSE_STATUS_OK.equals(status)) {
                        if (jsonObject.containsKey(BAIDU_URL_RESPONSE_RESULT)) {
                            ...
                            if (size > 0) {
                                for (int i = 0; i < size; i++) {
                                    ...
                                    if (coordinates.containsKey(BAIDU_GEO_CONV_URL_RESULT_LONGITUDE)) {
                                        ...
                                        if (StringUtils.isNotBlank(longitude)) {
                                            ...
                                            log.error("=== parseResponseOfGeoConv | 字段{}的值为空 ===", BAIDU_GEO_CONV_URL_RESULT_LONGITUDE);
                                        }
                                    } else {
                                    ...
                                    if (coordinates.containsKey(BAIDU_GEO_CONV_URL_RESULT_LATITUDE)) {
                                        ...
                                        if (StringUtils.isNotBlank(latitude)) {
                                            ...
                                            log.error("=== parseResponseOfGeoConv | 字段{}的值为空 ===", BAIDU_GEO_CONV_URL_RESULT_LATITUDE);
                                        }
                                    } else {
                                ...
                                log.error("=== parseResponseOfGeoConv | 结果集为空 ===");
                            }
                        } else {
